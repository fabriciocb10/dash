...
function userController($scope, api, users, arrayUtil, editableOptions, editableThemes, $filter) {
	...
	function createUser (_newUserForm) {
		$scope.formError.duplicateCedula = arrayUtil.findEqualObjByKey(users.data, 'cedula', $scope.newUser.cedula).founded,
		$scope.formError.duplicateEmail = arrayUtil.findEqualObjByKey(users.data, 'email', $scope.newUser.email).founded;

		if ($scope.formError.duplicateEmail || $scope.formError.duplicateCedula) {
			return;
		}

		if (_newUserForm.$valid && $scope.newUser.rol) {
